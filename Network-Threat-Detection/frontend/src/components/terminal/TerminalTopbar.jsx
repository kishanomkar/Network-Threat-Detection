...
export default function TerminalTopbar({ title }) {
  ...
  return (
    <header className="min-h-[5.75rem] bg-[#091229] border-b border-slate-200 px-5 lg:px-7 flex items-center justify-between sticky top-0 z-30 text-xs gap-5">
      <div className="flex flex-1 items-center gap-3 lg:gap-5 min-w-0">
        <h1 className="min-w-0 truncate text-[1.18rem] font-extrabold text-[#dce4ef] tracking-[-0.04em]">{title}</h1>
      ...
      <div className="flex items-center gap-3 lg:gap-5 shrink-0">
        <div className="hidden lg:flex items-center gap-1.5 text-[#dce4ef] text-xs"><Clock className="w-4 h-4" /><span>Last Analysis:</span><span className="font-semibold text-[#dce4ef]">{lastAnalysis ? lastAnalysis.toLocaleTimeString() : "Never"}</span></div>
        <button onClick={runFullAnalysis} disabled={analyzing} className="flex items-center gap-2 rounded-xl bg-[#FAF2CA] hover:[#FFFFFF] text-white px-4 py-3 font-bold tracking-wide transition disabled:opacity-50 shadow-[0_4px_12px_rgba(15,23,42,0.16)]">
          {analyzing ? <><RotateCw className="w-3.5 h-3.5 animate-spin" /><span>ANALYZING...</span></> : <><Play className="w-3.5 h-3.5 fill-current text-[#091229]" /><span className="text-[#091229]">RUN ANALYSIS</span></>}
        </button>
